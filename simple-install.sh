...
# An ICPipeline installer that minimizes user interaction requirements.
# It installs a complete framework in Public Network Mode, using default settings -- simplicity is the idea.
...
# omitting the full disclosure blurb module for simple install.
# source ./resources/include/full_disclosure.sh

# ***********************************************************************
# ***********************************************************************
# VARIABLES SPECIFIC TO SIMPLE INSTALL ARE GROUPED HERE, STILL CHANGEABLE
# EVEN THOUGH THEY'RE REMOVED FROM THE RUNTIME WORKFLOW:
# ***********************************************************************
# ***********************************************************************

# Public network mode is default/only option here (i.e. don't change this one).
# Please use the main installer for your Private Network Mode implementation.
public_network_mode=true


# Likewise with Worker ingress ports -- this option is removed from the runtime
# workflow, but you can still add ports here (the defaults are required by the framework itself).
worker_ingress_ports_array=(22 8080 65000 8090)

# set Worker count here:
number_of_workers=2

# ***********************************************************************
# END SIMPLE INSTALL VARS GROUPING
# ***********************************************************************

# Validate user-input CIDR notation (cheers to Mark Hatton in UK for the nice regex)
...
# The reset/uninstall script is compiled in reverse as the installer runs,
# and then flipped into forward order at the end.
...
# Each install generates its own randomized token
# for authenticating ICPL worker<>manager calls.  It's mandatory, so bail on fail.
...
# set OK flag before system checks
system_checks_ok=true

echo
echo -e "${cyan}******************************************************************************${clear}"
echo
echo "This is the simplified version of the ICPipeline installer.  It creates a complete implementation of the"
echo "ICPipeline framework, in Public Network Mode with default settings"
echo
echo "This installer creates two Worker replica nodes with public IP addresses."
echo "Workers run Ubuntu Linux with key-based SSH login configured."
echo "Password authentication is disabled by default, but you'll have the option of enabling it."
echo
echo "This simple install is generally suited for demo purposes, and help you get started."
echo "The results can be uninstalled completely by running ICPipeline Reset."
echo "Refer to the README for more information on Reset, which is also pretty straightforward."
echo
echo -e "${yellow}For fully secured, production-level ICPipeline implementations, please use the main installer instead.${clear}"
echo
echo -e "${cyan}******************************************************************************${clear}"
echo
pauseforuser
...
echo
echo "Installer is verifying your AWS profile configuration ..."
echo
...
echo -e "The installer refers to these settings when deploying your ${cyanbold}ICPipeline Workers${clear}"
echo "and supporting infrastructure."
echo
...
if [[ ! $(echo ${aws_cli_version} | grep 'aws-cli/2.') ]]; then
  ...
  echo "CLI Version 1 may in fact work, we're not doing anything particularly exotic here."
  echo "But be aware that we built and tested solely using CLI V2."
  echo
  echo "We recommend that you leave this here, and upgrade your AWS CLI to version 2 before proceeding."
  echo
  echo "For your reference:"
  echo "https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html"
  echo
  echo "Or else you can let it ride with version 1, your call.  You could even touch base and let us know how it works, which we'd appreciate."
...
echo
echo "Please take a moment to confirm that your AWS profile settings are OK."
echo "This determines where your AWS resources will end up -- account, region, etc."
echo
pauseforuser
...
echo
echo "Installer will verify that you have the required tools installed ..."
echo
...
# If the dfx canister sdk is not present, we take a stab at guiding through it.
...
then
  ...
  echo "We seem to be missing JQ, which is needed for handling JSON in bash."
  echo
...
# Note that simplified install is less stringent with token validation.  One may be supplied or not,
# but we're only confirming and verifying a match in this version.
isgood=0
while [[ $isgood == 0 ]]; do
...
# write final GH token to .env (which is how it's passed over to the Worker config on the other end)
...
echo "Please choose whether you'd also like to enable password authentication on your Workers."
echo
echo -e "Note that key-based authentication is automatically configured on each ${cyan}Worker${clear}."
echo "The installer generates a key pair and places the private key in /resources/worker-ssh-key/."
echo
echo -e "All your ${cyan}Workers${clear} will accept that key, which is specific to this installation."
echo

isgood=0
while [[ $isgood == 0 ]]; do
  echo
  read -p "Enable password authentication? (type and enter \"ENABLE\", or press <ENTER> to disable): " enable_worker_password_auth
  enable_worker_password_auth=${enable_worker_password_auth:-'NO'} && enable_worker_password_auth=$(echo "${enable_worker_password_auth}" | tr 'A-Z' 'a-z')
  while [[ ! ${enable_worker_password_auth} == "no" && ! ${enable_worker_password_auth} == "enable" ]]; do invalid_response; done
  if [[ ${enable_worker_password_auth} == "no" ||  ${enable_worker_password_auth} == "enable" ]]; then isgood=1; fi
done

if [[ "enable" == "${enable_worker_password_auth}" ]]
then
...
# RSA works if preferred -- marginally less secure and longer key length,
# but theoretically more backward-compatible.
# ssh-keygen -t rsa -b 4096 -f ./${worker_ssh_keyfile_name} -q -N ""

# Copy public keyfile into the Worker Docker build
...
# read in ports from var set above.  if needed, ports can be added to the
# worker_ingress_ports_array[] var at top.
ingress_ports_array=${worker_ingress_ports_array[*]}

# still scrubbing for dupes, which could still be mistyped in the header var ...
...
echo "Calling the network infrastructure module ..."
sleep 2

# *************************************************************************************************
# Call main component modules 
# *************************************************************************************************

# the AWS network infra module takes "include" as an arg,
# denoting that it's sourced by the installer as opposed to standalone (which also works).
# NOTE the simple installer calls a "simple" version of AWS infra module.
source ${resources_dir}/include/create_network_infra_aws_simple.sh include || exit_on_err "AWS infrastructure module not found, please check your project directory structure."

echo
echo "Calling the ICPM builder module ..."
echo
...
# worker.config.env file is complete by this point, copy into the Worker Docker build module.
cp -f "${dotenv_file}" "${icpw_dir}"|| exit_on_err "Installer encountered a problem copying worker.config.env into Docker build."

# here we run the container infra module.
# for simple install we eliminate user workflow/options for Fargate vs EC2 etc.
# defaulting to a Fargate cluster in a single public-subnet VPC, Workers publicly IP-addressed.
echo -e "The installer will build and push your ${cyan}Worker${clear} Docker image,"
echo -e "and run your ${cyan}ICPipeline Workers${clear} on AWS Fargate."
echo
sleep 2
...
if [[ false == "${enable_worker_password_auth}" ]]
then
  echo -e "You wisely chose to disable password authentication on your ${cyan}Worker${clear} containers."
else
  echo -e "${yellow}Password authentication will be enabled on your Workers${clear}."
  echo -e "Key-based auth is also configured and available on each ${cyan}Worker${clear}."
fi
...
echo "It's a bit more involved than the usual \"docker run\", and it takes just a minute or two."
