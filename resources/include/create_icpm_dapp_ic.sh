...
echo "including initialization fees.  ICPM is a two-canister architecture.  So the installer"
echo "is verifying your wallet balance of 8T or more for a smooth ICPM installation."
...
cycles_balance=${cycles_balance_str%%.*}
...
# if (( "${cycles_balance}" <= 14000000000000 && "${cycles_balance}" >= 12000000000000 ))
if (( "${cycles_balance}" <= 10 && "${cycles_balance}" >= 8 ))
then

  echo -e "${yellow}Your wallet's cycles balance of ${cycles_balance} is enough to"
...
# elif (( "${cycles_balance}" < 12000000000000 ))
elif (( "${cycles_balance}" < 8 ))
then
  
  echo -e "${red}Your wallet's cycles balance of ${cycles_balance} is not enough"
  echo -e "to create and fund the three canisters required for your Pipeline Manager deployment.${clear}"
  ...
  echo
  echo -e "${green}Your cycles wallet balance of about ${cycles_balance}T cycles is more"
...
# archive_canister_id=$(jq -r '.icpm_archive.ic' ${icpm_dir}/canister_ids.json)
echo "Pipeline Manager data canister ID: ${data_canister_id}"
echo "Pipeline Manager assets canister ID: ${assets_canister_id}"
# echo "Pipeline Manager archive canister ID: ${archive_canister_id}"
...
# echo -n "ICPM_BACKUP_CANISTER_ID=$archive_canister_id" | xargs >> "${dotenv_file}"
...
if [[ false == "${icpm_isresponsive}"  ]]; then
  ...
  then
    ...
    echo "Thank you.  Proceeding with stop/delete of remnant canisters..."
    echo "Stopping icpm canister ${data_canister_id}..."
    (cd "${icpm_dir}" && exec dfx canister --network ic stop "${data_canister_id}")
    echo "Deleting icpm canister ${data_canister_id}..."
    (cd "${icpm_dir}" && exec dfx canister --network ic delete "${data_canister_id}")
    echo "Stopping icpm_assets canister ${assets_canister_id}..."
    (cd "${icpm_dir}" && exec dfx canister --network ic stop "${assets_canister_id}")
    echo "Deleting icpm_assets canister ${assets_canister_id}..."
    (cd "${icpm_dir}" && exec dfx canister --network ic delete "${assets_canister_id}")
    # echo "Stopping icpm_archive canister ${archive_canister_id}..."
    # (cd "${icpm_dir}" && exec dfx canister --network ic stop "${archive_canister_id}")
    # echo "Deleting icpm_archive canister ${archive_canister_id}..."
    # (cd "${icpm_dir}" && exec dfx canister --network ic delete "${archive_canister_id}")
    ...
    echo "dfx canister --network ic stop ${assets_canister_id}"
    echo "dfx canister --network ic delete ${assets_canister_id}"
    # echo "dfx canister --network ic stop ${archive_canister_id}"
    # echo "dfx canister --network ic delete ${archive_canister_id}"
